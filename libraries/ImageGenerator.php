...
class ImageGenerator{
	const IMAGE_DIR = __DIR__ . '/../public/imagecache/'; // Where to cache the images.
	const IMAGE_CACHE_IN_SECONDS = 60;

	/*
		Function: handleRequest()
		Description: Main controller function. Validates input, and returns a new password.
	*/
	public function handleRequest($input = []){
		...
		// If there are no errors for photo, and a photo was submitted, process the photo.
		if(!array_key_exists('photo', $output['errors']) && $output['photo']){
			$this->removeOldFiles(); // Removes images that have expired in the cache.

			$dirname = (time() + self::IMAGE_CACHE_IN_SECONDS) . '-' . microtime(true) . rand(0,100000);
			mkdir(self::IMAGE_DIR . $dirname); // Makes a directory that will will expire in 60 seconds.
	...
	/*
		...
		Description: 
		Loop through all the directories in the cache. Remove expired ones.
	*/
